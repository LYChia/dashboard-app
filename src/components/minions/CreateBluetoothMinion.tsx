...
import { DeviceKind, BluetoothDevice } from "../../infrastructure/generated/api";
...
/**
 * The sort formula for sorting devices by name -> address -> uuid
 */
...
interface createBluetoothMinionInputProps {
	children: JSX.Element;
	text: string;
	disabled?: boolean;
}

function CreateBluetoothMinionInput(props: createBluetoothMinionInputProps) {
...
export function CreateBluetoothMinion() {
	...
	useEffect(() => {
		...
		(async () => {
			try {
				...
				const devices = await bluetoothService.getData();
	 ...
	 */
	async function createBluetoothMinion() {
		// If not ready, abort
		if (!validateInputs()) {
			return;
		}
		...
		try {
			await ApiFacade.MinionsApi.createBluetoothMinion({
				name,
				minionStatus: {},
		...
		>
			...
			<div style={{ width: '100%', maxHeight: 'calc(100% - 100px)', overflowY: 'auto', overflowX: 'hidden' }}>
				<Grid
					...
					alignItems="center"
				>
					<CreateBluetoothMinionInput text={t('dashboard.minions.create.new.name')}>
						<TextField
							style={{ width: `100%` }}
							disabled={creating}
							error={nameError}
						...
						/>
					</CreateBluetoothMinionInput>
					<CreateBluetoothMinionInput text={t('dashboard.minions.create.new.device')}>
						<Autocomplete
							options={devices}
							disabled={creating}
							getOptionLabel={(option: BluetoothDevice) => `${option.name} ${option.address} ${option.uuid}`}
							clearText={t('global.clear')}
							closeText={t('global.close')}
							noOptionsText={t('global.no.option')}
							onChange={(e, o) => {
								setDeviceError(false);
								setDevice(o as BluetoothDevice);
							}}
							renderInput={(params) => (
								<TextField
									{...params}
					...
					<CreateBluetoothMinionInput text={t('dashboard.minions.create.new.brand')}>
						<Autocomplete
					...
					<CreateBluetoothMinionInput text={t('dashboard.minions.create.new.model')}>
						<Autocomplete
